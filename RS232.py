...
class CEEO_RS232():
    ...
    async def on_ble_load(self, event):
        if self.uboard.connected:
            #import bledevice 
            import SPIKEbleCEEO, MIDI
            success = await self.uboard.board.upload('BLE_CEEO.py', SPIKEbleCEEO.code)
            success = await self.uboard.board.upload('MIDI.py', MIDI.code)
            #success = await self.uboard.board.upload('bledevice.py', bledevice.code)
            #self.python.code = bledevice.code
            await self.uboard.board.reset()
            self.uboard.focus()
